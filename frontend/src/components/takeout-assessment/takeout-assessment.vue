<template>
  <view class="takeout-assessment">
    <view class="assessment-header">
      ...
      <view class="header-title">外卖健康评估</view>
    </view>
    ...
    <view class="alternatives-section" v-if="foodData && showAlternatives">
      <view class="section-header">
        ...
        <view class="section-title">健康替代方案</view>
      </view>
      <view class="original-food">
        <text class="original-label">原菜品：</text>
        <text class="original-name">{{ foodData.name }}</text>
        <text class="original-score">（健康评分：{{ foodData.score }}分）</text>
      </view>
        ...
        >
          ...
          <view class="alternative-nutrition">
            <view class="nutrition-mini">
              <text class="mini-label">热量</text>
              <text class="mini-value">{{ alt.calories }} kcal</text>
            </view>
            <view class="nutrition-mini">
              <text class="mini-label">蛋白�?/text>
              <text class="mini-value">{{ alt.protein }}g</text>
            </view>
          </view>
    ...
    <view class="traceability-section" v-if="foodData && showTraceability">
      <view class="section-header">
        <view class="section-icon">🔍</view>
        <view class="section-title">食材来源追溯</view>
      </view>
      ...
      <view class="traceability-list">
        ...
        >
          ...
          <view class="certifications" v-if="item.certifications && item.certifications.length > 0">
            <view class="cert-title">�?食品安全认证�?/view>
...
export default {
  ...
  methods: {
    ...
    getRatingText(score) {
      if (score >= 90) return '优质健康食物'
      if (score >= 75) return '较健康食�?
      if (score >= 60) return '一般食�?
      if (score >= 40) return '需注意食物'
      return '不健康食�?
    },
    ...
    switchTab(tab) {
      this.activeTab = tab
      this.showAlternatives = tab === '替代方案'
      this.showTraceability = tab === '食材追溯'
      uni.vibrateShort()
    },
    ...
    viewAlternativeDetail(alt) {
      uni.showModal({
        title: alt.name,
        content: `健康评分�?{alt.score}分\n热量�?{alt.calories} kcal\n蛋白质：${alt.protein}g\n\n${alt.benefit}`,
        showCancel: false,
        confirmText: '知道�?
      })
    },
