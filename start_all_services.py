...
class ServiceManager:
    ...
    def _print_status(self):
        ...
        print("   厨艺团团: 健康饮食智能体系统后端服务 (路径: /tuantuan)")
        print("   外卖咕咕: 慧识外卖智能体服务 (路径: /takeout)")
        print("   健康糖豆: 健康管理智能体服务 (路径: /health)")
        print("   推荐小泽: 地图智能体服务 (路径: /map)")
        print("   慧识外卖智能体: 独立外卖分析服务 (端口: 5001)")
        print("   健康糖豆(独立): 健康管理独立服务 (端口: 8000)")
        print("   推荐小泽(独立): 地图智能体独立服务 (端口: 8002)")
        print("=" * 60 + "\n")
...
def validate_config_files(script_dir: Path) -> Tuple[bool, List[str]]:
    ...
    config_files = {
        ...
        "map-agent/main.py": script_dir / "frontend" / "map-agent" / "main.py",
...
def check_service_directories(script_dir: Path) -> Tuple[bool, List[str]]:
    ...
    directories = {
        ...
        "frontend/map-agent": script_dir / "frontend" / "map-agent",
...
def main():
    ...
    map_agent_service = Service(
        ...
        cwd=str(script_dir / "frontend" / "map-agent"),
    ...
    logger.info("   厨艺团团: http://localhost:5000/tuantuan")
    logger.info("   外卖咕咕: http://localhost:5000/takeout")
    logger.info("   健康糖豆: http://localhost:5000/health")
    logger.info("   推荐小泽: http://localhost:5000/map")
    logger.info("   慧识外卖智能体: http://localhost:5001")
    logger.info("   健康糖豆(独立): http://localhost:8000")
    logger.info("   推荐小泽(独立): http://localhost:8002")
    logger.info("=" * 60)
    logger.info("启动参数:")
    logger.info("   端口检查: ✅ 启用")
