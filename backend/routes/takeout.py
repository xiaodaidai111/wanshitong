...
try:
    from services.tuantuan_agent import VisionAnalyzer
    
    vision_analyzer = VisionAnalyzer(
        api_key=os.getenv('DASHSCOPE_API_KEY_TUANTUAN', ''),
        model_name="qwen-vl-plus",
        base_url="https://dashscope.aliyuncs.com/compatible-mode/v1"
    )
    logger.info("✅ 团团视觉识别模型已加载")
except Exception as e:
    vision_analyzer = None
    logger.warning(f"⚠️ 团团视觉识别模型加载失败: {str(e)}")

@takeout_bp.route('/')
def index():
    return jsonify({
        'service': '外卖咕咕',
        'description': '慧识外卖智能体服务',
        'endpoints': {
            'health': '/api/health - 健康检查',
            'diagnose': '/api/diagnose - 诊断服务',
            'chat': '/api/chat - 聊天对话',
            'image_analyze': '/api/image/analyze - 图像分析',
            'restaurant_score': '/api/restaurant/score - 餐厅评分',
...
def chat():
    ...
    return jsonify({
        'code': 0,
        'data': {
            'reply': f'收到您的消息：{message}，我是慧识营养师·咕咕，随时为您提供健康饮食建议！'
        }
    })
...
def analyze_image_with_tuantuan(image_bytes):
    ...
    try:
        ...
        result = vision_analyzer.analyze(image_base64=image_base64, prompt="请分析这道外卖菜品的食材、烹饪方式、营养均衡度，并给出健康评分和建议")
        return result
    except Exception as e:
        logger.error(f"团团视觉识别模型调用失败: {str(e)}")
        return None
