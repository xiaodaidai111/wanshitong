  ...
  <view class="repair-expert-view">
    ...
    <view class="content-container">
      <view class="expert-section">
        <image src="../../static/assistant-knowledge.png" mode="aspectFill" class="profile-avatar"></image>
        <view class="expert-dialogue-box">
        ...
        <scroll-view class="chat-history" scroll-y="true" :scroll-into-view="scrollTop">
          <view class="chat-message chef" id="msg-welcome">
            <view class="message-avatar">
              <image src="../../static/assistant-knowledge.png" mode="aspectFill" class="avatar-img"></image>
            </view>
        ...
        <view class="chat-input-container">
          <view class="input-wrapper">
            ...
            <view class="input-actions">
              ...
              <view class="quick-action" @click="insertQuickText('发动机启动困难怎么排查？')">💻</view>
              <view class="quick-action" @click="insertQuickText('发动机异响如何定位？')">💡</view>
            </view>
          </view>
        ...
        <view class="recipes-list">
          ...
          >
            <view class="recipe-image-wrap">
              <image
                :src="recipe.image ? '../../static/' + recipe.image : '../../static/equipment.png'"
                mode="aspectFill"
                class="recipe-image"
                :lazy-load="true"
              ></image>
            ...
            <view class="recipe-info">
              ...
              <view class="recipe-meta">
                ...
                <view class="recipe-calories">{{ recipe.difficulty || '中等' }}</view>
              </view>
            </view>
          </view>
        </view>
...
export default {
  ...
  data() {
    return {
      ingredient: '',
      cuisineIndex: 0,
      cuisineOptions: ['检修手册', '故障案例', '作业流程', '排查指南', '保养手册', '风险清单', '工具备件', '检修复盘', '经验总结'],
      showRecipe: false,
      recipeName: '',
      recipeImage: '',
      recipeIngredients: [],
  ...
  methods: {
    ...
    buildRecipeShareContent() {
      ...
      if (this.recipeNutrition) {
        sections.push(`检修目标：${this.recipeNutrition}`);
      }
      if (this.recipeTips) {
        sections.push(`安全复核提示：${this.recipeTips}`);
      }
    ...
    async loadLocalRecipes() {
      try {
        ...
        console.error('加载本地设备数据失败:', error);
      }
    },
    async fetchUserExp() {
      try {
    ...
    buildPageAgentContext() {
      return {
        page: '智能问修助手',
        current_input: this.ingredient,
        selected_resource_type: this.cuisineOptions[this.cuisineIndex] || '',
        generated_material: {
          name: this.recipeName,
          image: this.recipeImage,
          resources: this.recipeIngredients,
          steps: this.recipeSteps,
          summary: this.recipeNutrition,
          tips: this.recipeTips
        },
        recent_chat: this.chatHistory.slice(-6).map(item => ({
          role: item.type === 'user' ? 'user' : 'assistant',
          content: item.message
        })),
        local_materials: this.allRecipes.slice(0, 5).map(item => ({
    ...
    async sendMessage() {
      ...
      try {
        ...
        const response = await request.post('/cook-agent/chat', {
          message: userMsg,
          page_context: this.buildPageAgentContext()
        }, { service: 'tuantuan' });
...
.chat-message.expert .message-avatar {
  margin-right: 20rpx;
}
...
.chat-message.expert .message-text {
