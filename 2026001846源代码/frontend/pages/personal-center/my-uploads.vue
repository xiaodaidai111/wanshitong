...
export default {
  ...
  data() {
    return {
      tabs: [
        { key: 'all', label: '全部' },
        { key: 'health_records', label: '检修档案' },
        { key: 'takeaway_analysis', label: '检修记录' },
  ...
  methods: {
    ...
    getItemTitle(item) {
      if (item.type === 'health_records') {
        return `检修档案 - ${item.record_date}`
      } else if (item.type === 'takeaway_analysis') {
        return item.name || '检修记录'
    ...
    previewImage(imageSrc, title) {
      ...
      uni.navigateTo({
        url: '/pages/image-viewer/image-viewer?src=' + encodeURIComponent(imageSrc) + '&title=' + encodeURIComponent(title || '检修图片')
      });
    }
  }
}
