<template>
  <view class="solution-recommendation-view">
    ...
    <view class="content-container">
      <!-- 资源列表 -->
      <scroll-view class="solutions-scroll" scroll-y="true">
        <view class="solutions-list">
          <view
            v-for="(recipe, index) in currentSolutions"
            :key="recipe.id"
            class="solution-card"
            @click="viewRecipeDetail(recipe)"
          >
            <view class="solution-image-wrap">
              <image
              ...
              ></image>
              <view v-if="!recipe.is_liked" class="solution-rank-badge" :class="'rank-' + (index + 1)">
                {{ index + 1 }}
              </view>
            </view>
            <view class="solution-info">
              <view class="solution-name">{{ recipe.name }}</view>
              ...
              <view class="solution-desc">{{ recipe.description }}</view>
              <view class="solution-meta">
                <view class="solution-difficulty" :class="'difficulty-' + (recipe.difficulty || '中等')">
                  {{ getDifficultyText(recipe.difficulty) }}
                </view>
                <view class="solution-time">{{ recipe.cooking_time }}分钟</view>
                <view class="solution-score">{{ recipe.calories }}学习点</view>
              </view>
            </view>
          </view>
        </view>
        ...
        <!-- 空状态 -->
        <view class="empty-state" v-if="currentSolutions.length === 0 && !loading">
    ...
    <!-- 资源详情弹窗 -->
    <view class="modal-overlay" v-if="showSolutionDetail" @click="showSolutionDetail = false">
      <view class="solution-detail-modal" @click.stop>
        <view class="modal-drag-bar"></view>

        <view class="detail-header">
          <view class="detail-close-btn" @click="showSolutionDetail = false">
        ...
        <scroll-view class="detail-content" scroll-y="true">
          <image
            :src="getImageUrl(selectedSolution.image)"
            mode="aspectFill"
            class="detail-image"
            :lazy-load="true"
            @error="handleImageError"
          ...
          <view class="detail-info">
            <view class="detail-name">{{ selectedSolution.name }}</view>
            <view class="detail-description">{{ selectedSolution.description }}</view>

            <view class="detail-meta">
              <view class="detail-meta-item">
                <text class="meta-label">学习时长</text>
                <text class="meta-value">{{ selectedSolution.cooking_time }}分钟</text>
              </view>
              <view class="detail-meta-item">
                <text class="meta-label">难度</text>
                <text class="meta-value">{{ getDifficultyText(selectedSolution.difficulty) }}</text>
              </view>
              <view class="detail-meta-item">
                <text class="meta-label">学习点</text>
                <text class="meta-value">{{ selectedSolution.calories }}点</text>
              </view>
            </view>
            ...
            <view class="detail-rating">
              <view class="rating-stars">
                <text
                  ...
                  :key="i"
                  class="star"
                  :class="{ 'filled': i <= Math.round(selectedSolution.avg_rating) }"
                >
                  ★
                </text>
              </view>
              <text class="rating-text">{{ selectedSolution.avg_rating.toFixed(1) }}分</text>
              <text class="rating-count">({{ selectedSolution.rating_count }}人评分)</text>
            </view>
            ...
            <view class="detail-section">
              ...
              <view class="ingredients-list">
                <view
                  v-for="(ingredient, idx) in getIngredients(selectedSolution)"
                  :key="idx"
            ...
            <view class="detail-section">
              ...
              <view class="steps-list">
                <view
                  v-for="(step, idx) in getSteps(selectedSolution)"
                  :key="idx"
                  class="step-item"
                >
            ...
            <view class="detail-section">
              ...
              <view class="comments-section">
                <view
                  v-for="(comment, idx) in solutionComments"
                  :key="idx"
                  class="comment-item"
                >
        ...
        <view class="detail-footer">
          <view class="footer-action" @click="toggleLike(selectedSolution)">
            <text class="footer-icon">{{ selectedSolution.is_liked ? '❤️' : '🤍' }}</text>
            <text class="footer-text">{{ selectedSolution.is_liked ? '已收藏' : '收藏' }}</text>
          </view>
          <view class="footer-action primary" @click="openCommentModal(selectedSolution)">
...
export default {
  ...
  onLoad() {
    this.currentSolutions = [...this.allRecipes];
    
    // 初始化收藏列表，将所有已标记为收藏的资源添加到收藏列表中
    this.favoriteSolutions = this.allRecipes.filter(recipe => recipe.is_liked);
  },
  ...
  methods: {
    ...
    handleImageError(e) {
      ...
      e.target.src = '../../static/equipment.png';
    },
    ...
    filterRecipes() {
      ...
      switch(listType) {
        ...
        case 'economical':
          ...
          break;
        case 'favorites':
          filtered = [...this.favoriteSolutions];
          break;
      }

      this.currentSolutions = filtered;
      this.hasMore = false;
    },
    ...
    viewRecipeDetail(recipe) {
      this.selectedSolution = recipe;
      this.showSolutionDetail = true;
      this.solutionComments = [...this.mockComments];
    },
    ...
    toggleLike(recipe) {
      ...
      if (recipe.is_liked) {
        // 检查是否已经在收藏列表中，避免重复添加
        const existingIndex = this.favoriteSolutions.findIndex(r => r.id === recipe.id);
        if (existingIndex === -1) {
          // 添加到收藏列表
          this.favoriteSolutions.push(recipe);
          uni.showToast({
            title: '已收藏 ❤️',
            icon: 'none',
            duration: 1200
          ...
          // 已经在收藏列表中，更新状态
          this.favoriteSolutions[existingIndex].is_liked = true;
          uni.showToast({
            title: '已收藏 ❤️',
            icon: 'none',
            duration: 1200
        ...
        // 从收藏列表移除
        const index = this.favoriteSolutions.findIndex(r => r.id === recipe.id);
        if (index !== -1) {
          this.favoriteSolutions.splice(index, 1);
          uni.showToast({
            title: '已取消收藏',
            icon: 'none',
            duration: 1200
    ...
    submitComment() {
      ...
      // 添加到评价列表中
      this.solutionComments.unshift(newComment);
    ...
    updateRecipeRating(newRating) {
      const recipe = this.selectedSolution;
      if (!recipe) return;
      ...
      // 更新所有资源列表中的对应资源评分
      const recipeIndex = this.currentSolutions.findIndex(r => r.id === recipe.id);
      if (recipeIndex !== -1) {
        this.currentSolutions[recipeIndex].avg_rating = recipe.avg_rating;
        this.currentSolutions[recipeIndex].rating_count = recipe.rating_count;
      }
    },
...
.solution-rating .rating-stars {
  display: flex;
  gap: 2rpx;
}

.solution-rating .star {
  font-size: 20rpx;
  opacity: 0.3;
}

.solution-rating .star.filled {
  opacity: 1;
  color: #ffb800;
}

.solution-rating .rating-text {
  font-size: 22rpx;
  font-weight: 600;
  color: #ea580c;
}
...
.solution-difficulty.difficulty-简单{
  color: #16a34a;
  background: #f0fdf4;
}

.solution-difficulty.difficulty-中等 {
  color: #ea580c;
  background: #fff7ed;
}

.solution-difficulty.difficulty-困难 {
  color: #dc2626;
  background: #fef2f2;
}
